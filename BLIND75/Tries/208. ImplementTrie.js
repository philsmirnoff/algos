...
var Trie = function() {
    this.root = {}; // Initialize the root as an empty object
};
...
Trie.prototype.search = function(word) {
    ...
    for (let char of word) {
        if (!node[char]) {
            return false; // If at any point, character not found
        }
        node = node[char];
    }
    return node.isEnd === true; // Only return true if it's an end of a word
};
...
Trie.prototype.startsWith = function(prefix) {
    ...
    for (let char of prefix) {
        if (!node[char]) {
            return false; // If prefix path breaks
        }
        node = node[char];
    }
